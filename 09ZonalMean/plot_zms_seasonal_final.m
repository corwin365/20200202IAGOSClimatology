...
Scale = Data.Settings.AbsPrsScale;
...
for iSeason = 1:1:4;
  ...
  Plot.GWs( iSeason,:,:) = squeeze(nanmean(Data.Results.Abs( 2,ThisSeason,:,:),2));
  Plot.Wind(iSeason,:,:) = squeeze(nanmean(Data.Results.Abs( 1,ThisSeason,:,:),2));  
  Plot.N(   iSeason,:,:) = squeeze(nansum( Data.Results.AbsN(2,ThisSeason,:,:),2));
...
for iVariable=1:1:3;
  ...
  for iQuarter = 1:1:4;
    ...
    %plot tropause
    plot(Lat,Plot.TP(iQuarter,:),'k-','linewi',3)
    ...
    set(gca,'yscale','log');
    axis([min(Lat)-2.5 max(Lat)+2.5 min(Scale) max(Scale)])
...
%     if     iQuarter == 4; set(gca,'yaxislocation','right'); 
    ...
    if iVariable ~= 3; set(gca,'xticklabel',{});
